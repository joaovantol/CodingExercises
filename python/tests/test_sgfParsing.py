...
from codingexercises.sgfParsing import (
    parse,
    SgfTree,
)
...
class SgfParsingTest(unittest.TestCase):
    ...
    def test_tree_with_no_nodes(self) -> None:
        input_string = "()"
        with self.assertRaises(ValueError) as err:
            parse(input_string)
        self.assertEqual(type(err.exception), ValueError)
    ...
    def test_node_without_tree(self) -> None:
        input_string = ";"
        with self.assertRaises(ValueError) as err:
            parse(input_string)
        self.assertEqual(type(err.exception), ValueError)
    ...
    def test_properties_without_delimiter(self) -> None:
        input_string = "(;A)"
        with self.assertRaises(ValueError) as err:
            parse(input_string)
        self.assertEqual(type(err.exception), ValueError)
    ...
    def test_all_lowercase_property(self) -> None:
        input_string = "(;a[b])"
        with self.assertRaises(ValueError) as err:
            parse(input_string)
        self.assertEqual(type(err.exception), ValueError)
    ...
    def test_upper_and_lowercase_property(self) -> None:
        input_string = "(;Aa[b])"
        with self.assertRaises(ValueError) as err:
            parse(input_string)
        self.assertEqual(type(err.exception), ValueError)
    ...
    def test_two_child_trees(self) -> None:
    ...
    def test_multiple_property_values(self) -> None:
        input_string = "(;A[b][c][d])"
        expected = SgfTree(properties={"A": ["b", "c", "d"]})
        self.assertEqual(parse(input_string), expected)
    ...
    def test_within_property_values_newlines_remain_as_newlines(self) -> None:
    ...
    def test_escaped_backslash_in_property_value_becomes_just_a_backslash(self) -> None:
        input_string = "(;A[\\\\])"
        expected = SgfTree(properties={"A": ["\\"]})
        self.assertEqual(parse(input_string), expected)

    def test_opening_bracket_within_property_value_doesn_t_need_to_be_escaped(self) -> None:
    ...
    def test_semicolon_in_property_value_doesn_t_need_to_be_escaped(self) -> None:
    ...
    def test_parentheses_in_property_value_don_t_need_to_be_escaped(self) -> None:
        input_string = "(;A[x(y)z][foo]B[bar];C[baz])"
        expected = SgfTree(
            properties={"A": ["x(y)z", "foo"], "B": ["bar"]},
            children=[SgfTree({"C": ["baz"]})],
    ...
    def test_escaped_tab_in_property_value_is_converted_to_space(self) -> None:
        input_string = "(;A[hello\\\tworld])"
        expected = SgfTree(properties={"A": ["hello world"]})
        self.assertEqual(parse(input_string), expected)

    def test_escaped_newline_in_property_value_is_converted_to_nothing_at_all(self) -> None:
        input_string = "(;A[hello\\\nworld])"
        expected = SgfTree(properties={"A": ["helloworld"]})
        self.assertEqual(parse(input_string), expected)

    def test_escaped_t_and_n_in_property_value_are_just_letters_not_whitespace(self) -> None:
